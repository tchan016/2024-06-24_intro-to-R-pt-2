...
CCHS <- readr::read_csv(file = "C:/2013-2014 CCHS (PUMF)/cchs-82M0013-E-2013-2014-Annual-component_F1.csv", col_names = T, progress = T)
...
🚶‍♀️ Make a descriptive table with age, diabetes, and fruit, stratified by sex, but no overall column and now specifying median & IQR for continuous variables (hint: `render.continuous=c("Median [IQR]"="Median [Q1, Q3]")`) and also having these values be rounded to 2 significant digits (via `digits=2`)
...
T1Vars <- c('DHHGAGE', 'DHH_SEX', 'Diabetes', 'Heart', 'Fruit') 
...
T1CatVars <- c('DHHGAGE', 'DHH_SEX', 'Diabetes', 'Heart')
...
write.csv(T1out, file = "C:/Users/tchan/Downloads/tableone_overall_table.csv")
...
T1Vars <- c('DHHGAGE', 'DHH_SEX', 'Diabetes', 'Fruit') 
...
T1CatVars <- c('DHHGAGE', 'DHH_SEX', 'Diabetes')
...
write.csv(THeartout, file = "C:/Users/tchan/Downloads/tableone_stratified_by_sex.csv")
